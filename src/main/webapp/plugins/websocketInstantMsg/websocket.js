...
//启动聊天窗口并连接聊天服务器
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('ULadminqq9551942'+user);
					}
			...
			win.on("close",function(){
				websocket.send('LeaveULadminqq9551942');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "uladmin886"+toUser+"fhfhadmin888" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用 環球雲倉 即时通讯系统。</div>',
			'<tpl for=".">',
