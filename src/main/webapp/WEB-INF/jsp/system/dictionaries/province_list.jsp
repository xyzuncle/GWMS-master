...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="province/provincelist.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><a href="javascript:goSondict('${var.provinceid }')"><i class="ace-icon fa fa-share bigger-100"></i>&nbsp;${var.province}</a></td>
											<td class='center'><a href="javascript:goSondict('${var.provinceid }')">${var.provinceid}</a></td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>province/goProvinceAdd.do';
			 diag.Width = 500;
			 diag.Height = 500;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>province/deleteProvince.do?id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>province/goProvinceEdit.do?id='+Id;
			 diag.Width = 500;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
                 nextPage(${page.currentPage});
