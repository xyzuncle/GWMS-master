            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <!-- 检索  -->
                        <form action="sender/list.do" method="post" name="Form" id="Form">
                                ...
                                <tr>
                                    <td>
                                        <div class="nav-search">
										    ...
										    <span class="input-icon">
											    <input type="text" placeholder="这里输入关键词" class="nav-search-input" id="nav-search-sender" autocomplete="off" name="sender" value="${pd.sender }" placeholder="这里输入关键词"/>
										    </span>
                                        </div>
                                    </td>
                                    <c:if test="${QX.cha == 1 }">
                                        ...
                                        <c:if test="${QX.cha == 1 }">
                                            <c:forEach items="${varList}" var="var" varStatus="vs">
                                                <tr>
                                                    ...
                                                    <td class='center'>${var.senderprovince}</td>
                                                    <td class='center'>${var.sendercity}</td>
                                                    <td class="center">
...
<script type="text/javascript">
    ...
    function add(){
        ...
        diag.Drag=true;
        diag.Title ="新增";
        diag.URL = '<%=basePath%>sender/goAdd.do';
        diag.Width = 600;
        diag.Height = 500;
    ...
    function del(Id){
        bootbox.confirm("确定要删除吗?", function(result) {
            if(result) {
                top.jzts();
                var url = "<%=basePath%>sender/delete.do?id="+Id+"&tm="+new Date().getTime();
    ...
    function edit(Id){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>sender/goEdit.do?id='+Id;
        diag.Width = 600;
        diag.Height = 500;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
            ...
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要删除选中的数据吗?'){
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>sender/deleteAll.do?tm='+new Date().getTime(),
                            data: {DATA_IDS:str},
                            dataType:'json',
