package com.huanqiuyuncang.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("ULadminqq9551942")){
			this.userjoin(message.replaceFirst("ULadminqq9551942", ""),conn);
		}if(null != message && message.startsWith("LeaveULadminqq9551942")){
			this.userLeave(conn);
		}if(null != message && message.contains("uladmin886")){
			String toUser = message.substring(message.indexOf("uladmin886")+10, message.indexOf("fhfhadmin888"));
			message = message.substring(0, message.indexOf("uladmin886")) +"[私信]  "+ message.substring(message.indexOf("fhfhadmin888")+12, message.length());
